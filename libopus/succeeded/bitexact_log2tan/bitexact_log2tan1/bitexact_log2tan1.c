...
int LLVMFuzzerTestOneInput(uint8_t * Fuzz_Data, size_t Fuzz_Size){
    ...
    //GEN_BUILTIN
    int b_isin;
    memcpy(&b_isin, futag_pos, sizeof(int));
    futag_pos += sizeof(int);
    
    //GEN_BUILTIN
    int b_icos;
    memcpy(&b_icos, futag_pos, sizeof(int));
    futag_pos += sizeof(int);
    
    //FUNCTION_CALL
    bitexact_log2tan(b_isin ,b_icos );
    //FREE
    return 0;
}
// Compile database: 
