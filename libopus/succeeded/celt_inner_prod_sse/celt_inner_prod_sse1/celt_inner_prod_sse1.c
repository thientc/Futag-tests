...
int LLVMFuzzerTestOneInput(uint8_t * Fuzz_Data, size_t Fuzz_Size){
    if (Fuzz_Size < 0*sizeof(wchar_t) + 0*sizeof(char) + 0 + 0 + sizeof(opus_val16) + sizeof(opus_val16) + sizeof(int)) return 0;
    ...
    //FUNCTION_CALL
    celt_inner_prod_sse(p_q_b__x ,p_q_b__y ,b_N );
    //FREE
    return 0;
}
// Compile database: 
